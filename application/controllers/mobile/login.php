...
class login extends CI_Controller {
	...
	function on(){
		...
		$this->savelog( $Email_txt=$user,'Usuario autenticado Contentstuff:Mobile',json_encode($dados) , json_encode($result->CSWF_TabletLoginExternoBooleanResult)  );
		...
		if ( $result->CSWF_TabletLoginExternoBooleanResult ){
			...
			$this->savelog( $Email_txt=$user ,'Importando titulos Contentstuff:Mobile',json_encode($dados),json_encode($edicao) );
			...
			$this -> parser -> parse('mobile/edicao_download', $this->data);	

			
		}else{
			
			$this->data['msg'] = 'Usuário ou senha invalido' ;
			$this -> parser -> parse('mobile/login', $this->data);	
		}
	...
	function showdpf() {
		...
		$senha = $this->decrypt( urldecode( $this->uri->segment("6") ) , '10101972') ;

		$dados = array( 'Email_txt' => $usuario) ;
		$this->savelog( $usuario,'Download:Mobile',json_encode($dados), $id  );
		
		$this->load->helper('download');

		$file = base_url()."acervo/pdf/$id.pdf";

		$file= realpath("acervo/pdf/$id.pdf");
	...
	function showpdfhtml(){
		...
		$dados = array( 'Email_txt' => $usuario) ;
		$this->savelog( $usuario,'Lendo:Mobile',json_encode($dados), $id  );
