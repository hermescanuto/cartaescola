...
class Upload_imagem  extends CI_Controller {
	...
	function novo()
	{
		...
		$pic[2] = 'da matéria aberta 446px por 283px 75dpi ';
		...
		if ( $this->table == 1 ){
			$data["imagem"] = $recordset["imagem_home"] ;
		}

		if ( $this->table == 2 ){
			$data["imagem"] = $recordset["imagem_fundo"] ;
		}

		if ( $this->table == 3  ){
			$data["imagem"] = base_url().'upload/capa/'.$recordset["imagem_capa"] ;
		...
		$this->parser->parse( 'util/upload_imagem',$data);
	...
	function do_upload( )
	{
		...
		else
		{
			...
			$this->parser->parse( 'util/upload_imagem_ok',$data);
