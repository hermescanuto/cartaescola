...
class Form extends CI_Controller {
    const IMAGE_UPLOAD_DIR = '/upload2';
    ...
    public function index() {
        $this -> load -> view('form', array('ckeditor' => $this -> _setup_ckeditor('content'), 'content_html' => '' // your model property's HTML for CKEditor textarea
        ));
    }
    ...
    private function _move_image($temp_location) {
        ...
        if (isset($temp_location['tmp_name']) && isset($info['extension']) && in_array($ext, $this -> _supported_extensions)) {
            $new_file_path = self::IMAGE_UPLOAD_DIR . "/$filename";
            if (!is_dir(self::IMAGE_UPLOAD_DIR) || !is_writable(self::IMAGE_UPLOAD_DIR)) {
    ...
    private function _setup_ckeditor($id) {
        ...
        $ckeditor = array('id' => $id, 'path' => 'assets/js/ckeditor', 'config' => array('toolbar' => 'Full', 'width' => '800px', 'height' => '400px', 'filebrowserImageUploadUrl' => '/form/upload'));
