...
class CI_Pagination {
	...
	function create_links($search)
	{
			...
			for ($loop = $start -1; $loop <= $end; $loop++)
			{
				...
				if ($i >= $base_page)
				{
					...
					else
					{
						...
						if ($n == '' && $this->first_url != '')
						{
							$output .= $this->num_tag_open.'<a '.$this->anchor_class.'href="'.$this->first_url.'/0/'.$search.'">'.$loop.'</a>'.$this->num_tag_close;
						}
						else
						{
		...
		if ($this->next_link !== FALSE AND $this->cur_page < $num_pages)
		{
			...
			$output .= $this->next_tag_open.'<a '.$this->anchor_class.'href="'.$this->base_url.$this->prefix.$i.$this->suffix.'/'.$search.'">'.$this->next_link.'</a>'.$this->next_tag_close;
		}
		...
		if ($this->last_link !== FALSE AND ($this->cur_page + $this->num_links) < $num_pages)
		{
			...
			$output .= $this->last_tag_open.'<a '.$this->anchor_class.'href="'.$this->base_url.$this->prefix.$i.$this->suffix.'/'.$search.'">'.$this->last_link.'</a>'.$this->last_tag_close;
		}
